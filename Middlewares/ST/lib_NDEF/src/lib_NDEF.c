...
static uint16_t NDEF_ParseRecordHeader ( struct sRecordInfo *pRecordStruct );
static void NDEF_ParseWellKnownType ( struct sRecordInfo *pRecordStruct );
static void NDEF_ParseMediaType ( struct sRecordInfo *pRecordStruct );
static void NDEF_ParseForumExternalType ( struct sRecordInfo *pRecordStruct );
static void NDEF_ParseURI(struct sRecordInfo *pRecordStruct);
static void NDEF_ParseSP(struct sRecordInfo *pRecordStruct);
static uint16_t NDEF_IdentifySPRecord ( struct sRecordInfo *pRecordStruct, uint8_t* pPayload );

/* In case of smart Poster composed with different record, 3 records supported so far */
struct sRecordInfo SPRecordStruct1, SPRecordStruct2, SPRecordStruct3;
struct sRecordInfo *SPRecordStructAdd[SP_MAX_RECORD] = { &SPRecordStruct1, &SPRecordStruct2, &SPRecordStruct3 };
...
static void NDEF_ParseSP(struct sRecordInfo *pRecordStruct)
{
	...
	struct sRecordInfo *pSPRecordStruct;
...
uint16_t NDEF_IdentifyNDEF ( struct  sRecordInfo *pRecordStruct, uint8_t* pNDEF )
{
	...
	if( pNDEF[0]&SR_Mask)
	{
          ...
          {
            //For Other Wifi Credentials Parsing
