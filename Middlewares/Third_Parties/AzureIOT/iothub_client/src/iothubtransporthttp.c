    ...
    else
    {
        ...
        if (handleData->doBatchedTransfers)
        {
            ...
            else
            {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                if (statusCode < 300)
                                {
                                    /*Codes_SRS_TRANSPORTMULTITHTTP_17_070: [If HTTPAPIEX_SAS_ExecuteRequest does not fail and http status code <300 then IoTHubTransportHttp_DoWork shall call IoTHubClient_LL_SendComplete. Parameter PDLIST_ENTRY completed shall point to a list containing all the items batched, and parameter IOTHUB_CLIENT_CONFIRMATION_RESULT result shall be set to IOTHUB_CLIENT_CONFIRMATION_SUCCESS. The batched items shall be removed from waitingToSend.] */
                                    IoTHubClient_LL_SendComplete(iotHubClientHandle, &(deviceData->eventConfirmations), IOTHUB_CLIENT_CONFIRMATION_SUCCESS);
                                }
                                else
                                {
                                    //items go back to waitingToSend
                ...
                {
                    IoTHubClient_LL_SendComplete(iotHubClientHandle, &(deviceData->eventConfirmations), IOTHUB_CLIENT_CONFIRMATION_ERR); /*takes care of emptying the list too*/
                    break;
                }
                                    ...
                                    else
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if (r == HTTPAPIEX_OK)
                                            {
                                                if (statusCode < 300)
                                                {
                                                    /*Codes_SRS_TRANSPORTMULTITHTTP_17_082: [If HTTPAPIEX_SAS_ExecuteRequest does not fail and http status code <300 then IoTHubTransportHttp_DoWork shall call IoTHubClient_LL_SendComplete. Parameter PDLIST_ENTRY completed shall point to a list the item send, and parameter IOTHUB_CLIENT_CONFIRMATION_RESULT result shall be set to IOTHUB_CLIENT_CONFIRMATION_SUCCESS. The item shall be removed from waitingToSend.] */
                                                    ...
                                                    IoTHubClient_LL_SendComplete(iotHubClientHandle, &(deviceData->eventConfirmations), IOTHUB_CLIENT_CONFIRMATION_SUCCESS); /*takes care of emptying the list too*/
                                                }
                                                else
                                                {
