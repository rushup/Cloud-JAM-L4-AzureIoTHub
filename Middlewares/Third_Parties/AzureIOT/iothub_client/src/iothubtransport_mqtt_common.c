                ...
                if (puback != NULL)
                {
                    ...
                    while (currentListEntry != &transport_data->telemetry_waitingForAck)
                    {
                        ...
                        if (puback->packetId == mqttMsgEntry->packet_id)
                        {
                            ...
                            sendMsgComplete(mqttMsgEntry->iotHubMessageEntry, transport_data, IOTHUB_CLIENT_CONFIRMATION_SUCCESS);
                            free(mqttMsgEntry);
                        }
                    ...
                    if (((current_ms - mqttMsgEntry->msgPublishTime) / 1000) > RESEND_TIMEOUT_VALUE_MIN)
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    (void)DList_RemoveEntryList(currentListEntry);
                                    sendMsgComplete(mqttMsgEntry->iotHubMessageEntry, transport_data, IOTHUB_CLIENT_CONFIRMATION_ERR);
                                    free(mqttMsgEntry);
                                }
                            }
                        }
                ...
                while (currentListEntry != transport_data->waitingToSend)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                (void)(DList_RemoveEntryList(currentListEntry));
                                sendMsgComplete(iothubMsgList, transport_data, IOTHUB_CLIENT_CONFIRMATION_ERR);
                                free(mqttMsgEntry);
                            }
                            else
                            {
